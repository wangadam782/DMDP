...
def collect_rollout(
...
) -> tuple[dict[str, np.ndarray], Any, dict[str, Any], list[dict[str, float]], int, dict[str, Any]]:
    ...
    last_terms: dict[str, Any] = {"w2": 0.0, "tail_risk": 0.0}
    ...
    for _ in range(rollout_steps):
        ...
        omega, w2, risk = distribution_terms(features, feature_names, target, feature_cfg)
        omega_agents = np.repeat(omega[None, :], obs_agents.shape[0], axis=0).astype(np.float32)
        values = agent.value(obs_agents, omega_agents, critic_obs_agents).astype(np.float32)
        ...
        shaped_rewards = (
            reward_arr
            - float(reward_cfg.get("cost_penalty", 0.1)) * cost_arr
            - float(reward_cfg.get("distribution_penalty", 0.05)) * (w2 / n_agents)
            - float(reward_cfg.get("tail_penalty", 0.2)) * (risk / n_agents)
        ).astype(np.float32)
        ...
        episode_state["train_return"] += float(np.sum(shaped_rewards))
        ...
        obs = next_obs
        info = next_info
        last_terms = {"w2": w2, "tail_risk": risk}
            ...
            completed_episodes.append(
                {
                    ...
                    "episode_state_w2": float(episode_state["w2"] / length),
                    "episode_tail_risk": float(episode_state["tail_risk"] / length),
                }
            )
            obs, info = env.reset()
            episode_state.update({"return": 0.0, "train_return": 0.0, "cost": 0.0, "w2": 0.0, "tail_risk": 0.0, "length": 0})
    ...
    if done_steps and bool(np.any(done_steps[-1])):
        ...
        next_features = extract_state_features(obs, info, feature_cfg)
        next_omega, _, _ = distribution_terms(next_features, feature_names, target, feature_cfg)
        next_omega_agents = np.repeat(next_omega[None, :], next_obs_agents.shape[0], axis=0).astype(np.float32)
        last_values = agent.value(next_obs_agents, next_omega_agents, next_critic_obs_agents).astype(np.float32)
    ...
    return batch, obs, completed_episodes, len(reward_steps), info, last_terms
...
def save_checkpoint(agent: DMDPMAPPO, path: str | Path, metadata: dict[str, Any]) -> None:
    path = Path(path)
...
def main() -> None:
    ...
    parser.add_argument("--algo-config", default="configs/dmdp_mappo.yaml")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--target", default=None)
    parser.add_argument("--total-steps", type=int, default=None)
    parser.add_argument("--rollout-steps", type=int, default=None)
    parser.add_argument("--run-dir", default=None, help="Directory for this DMDP-MAPPO run.")
    ...
    method_name = str(algo_config.get("method", "dmdp_mappo"))
    ...
    if args.run_dir is not None:
        ...
        artifact_prefix = "method5" if method_name.startswith("method5") else "dmdp_mappo"
    ...
    try:
        ...
        episode_state = {"return": 0.0, "train_return": 0.0, "cost": 0.0, "w2": 0.0, "tail_risk": 0.0, "length": 0}
        episodes: list[dict[str, float]] = []
        update_rows: list[dict[str, float]] = []
        steps_done = 0

        for update_idx in range(updates):
            batch, obs, completed, env_steps, info, last_terms = collect_rollout(
                env,
                agent,
                obs,
                info,
        ...
        checkpoint_path = output_cfg.get("checkpoint", "outputs/checkpoints/dmdp_mappo_latest.pt")
        metrics_path = output_cfg.get("metrics", "outputs/metrics/dmdp_mappo_train.json")
        metadata = {
            "method": method_name,
        ...
        metrics = {
            "method": method_name,
            "status": "trained",
            "state_distribution_control": "omega=[mu,sigma]",
