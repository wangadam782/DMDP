...
RUNS = [
    (
        "Method 1 State-MAPPO",
        "outputs/runs/mappo_method1_multigoal_100k_ccritic/eval/mappo_eval.json",
    ),
    (
        "Method 2 Dist-MAPPO",
        "outputs/runs/dist_mappo_method2_100k_ccritic/eval/dist_mappo_eval.json",
    ),
    (
        "Method 3 DMDP-MAPPO",
        "outputs/runs/dmdp_method3_100k_ccritic/eval/dmdp_eval.json",
    ),
    (
        "Method 4 Online",
        "outputs/runs/dmdp_paper_online_multienv_100k_fixed/eval/dmdp_paper_online_eval.json",
    ),
    (
        "Method 4 Tail Warmup",
        "outputs/runs/dmdp_paper_online_tail_warmup_100k/eval/dmdp_paper_online_eval.json",
    ),
    (
        "Method 4 Residual Warmup",
        "outputs/runs/dmdp_paper_online_residual_warmup_100k/eval/dmdp_paper_online_eval.json",
    ),
]
...
def plot_metric(rows: list[dict[str, Any]], key: str, title: str, direction: str, output: Path) -> None:
    ...
    colors = ["#4C78A8", "#72B7B2", "#54A24B", "#E45756", "#F58518", "#B279A2"]

    fig, ax = plt.subplots(figsize=(10, 5.4))
    bars = ax.bar(labels, values, color=colors[: len(values)], width=0.72)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/comparisons/four_methods_with_method4_residual_warmup_100k")
