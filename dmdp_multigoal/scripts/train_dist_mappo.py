...
def collect_rollout(
    ...
    for _ in range(rollout_steps):
        ...
        values = agent.critic_metric(obs_agents, critic_obs_agents).astype(np.float32)
    ...
    if done_steps and bool(np.any(done_steps[-1])):
        ...
        last_values = agent.critic_metric(next_obs_agents, next_critic_obs_agents).astype(np.float32)
...
def save_checkpoint(agent: DistributionalMAPPO, path: str | Path, metadata: dict[str, Any]) -> None:
    path = Path(path)
...
def main() -> None:
    ...
    parser.add_argument("--algo-config", default="configs/dist_mappo.yaml")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--total-steps", type=int, default=None)
    parser.add_argument("--rollout-steps", type=int, default=None)
    parser.add_argument("--run-dir", default=None, help="Directory for this Distributional MAPPO run.")
    ...
    if args.run_dir is not None:
        ...
        algo_config["output"]["checkpoint"] = str(run_dir / "checkpoints" / "dist_mappo_latest.pt")
        algo_config["output"]["metrics"] = str(run_dir / "metrics" / "dist_mappo_train.json")
        save_json(run_dir / "configs" / "env_config.json", env_config)
        save_json(run_dir / "configs" / "dist_mappo_config.json", algo_config)
    ...
    try:
        ...
        agent = DistributionalMAPPO(obs_dim=obs_dim, action_dim=action_dim, config=algo_config, critic_obs_dim=critic_obs_dim)
        ...
        checkpoint_path = output_cfg.get("checkpoint", "outputs/checkpoints/dist_mappo_latest.pt")
        metrics_path = output_cfg.get("metrics", "outputs/metrics/dist_mappo_train.json")
        metadata = {
            "method": "dist_mappo",
