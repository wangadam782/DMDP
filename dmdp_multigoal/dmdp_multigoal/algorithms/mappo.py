...
class StateFeedbackMAPPO:
    """Minimal MAPPO-style PPO update over flattened agent-time samples."""
    ...
    def __init__(
        ...
        self.critic = ValueCritic(self.critic_obs_dim, hidden, activation=activation).to(self.device)
    ...
    def value(self, obs: np.ndarray, critic_obs: np.ndarray | None = None) -> np.ndarray:
    ...
    def update(self, batch: dict[str, np.ndarray]) -> dict[str, float]:
        ...
        for _ in range(num_epochs):
            ...
            value_loss = torch.nn.functional.mse_loss(self.critic(critic_obs), returns)
